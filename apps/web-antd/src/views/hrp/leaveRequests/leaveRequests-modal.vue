 ...
 * 定义默认值 用于reset
 */
const defaultValues: Partial<LeaveRequestsForm> = {
  id: undefined,
  userId: undefined,
  leaveDate: undefined,
 ...
 * 表单校验规则
 */
const formRules = ref<AntdFormRules<LeaveRequestsForm>>({
  userId: [{ required: true, message: '员工 ID不能为空' }],
  approvalStatus: [
    { required: true, message: '状态(字典: 已提交, 已锁定, 已取消)不能为空' },
  ],
...
const [BasicModal, modalApi] = useVbenModal({
  ...
  onOpenChange: async (isOpen) => {
    ...
    if (isUpdate.value && id) {
      const record = await leaveRequestsInfo(id);
      // 只赋值存在的字段
...
async function handleConfirm() {
  try {
    ...
    await (isUpdate.value ? leaveRequestsUpdate(data) : leaveRequestsAdd(data));
    resetInitialized();
    emit('reload');
    modalApi.close();
  } catch (error) {
...
<template>
  <BasicModal :title="title">
    <Form :label-col="{ span: 4 }">
      ...
      <FormItem label="期望休假日期" v-bind="validateInfos.leaveDate">
        <!-- 需要自行调整参数 -->
        <DatePicker
          v-model:value="formData.leaveDate"
