...
const dailyRequirements = reactive<ShiftRequirement[]>([
  {
    timeSlot: '10:30-18:00',
    positions: { 柜台: 1, 外场: 1, 内场: 1, 洗碗: 0, 清洁: 0 },
  },
  {
    timeSlot: '18:00-23:00',
    positions: { 柜台: 1, 外场: 1, 内场: 1, 洗碗: 1, 清洁: 1 },
  },
  {
    timeSlot: '20:00-23:00',
    positions: { 柜台: 0, 外场: 1, 内场: 0, 洗碗: 0, 清洁: 0 },
  },
]);
...
const generateSchedule = () => {
  ...
  setTimeout(() => {
    // 记录每个员工本周已安排的工作天数
    const workDaysCount = new Map<number, number>(
      mockEmployees.map((e) => [e.id, 0]),
    );

    // 1. 优先安排正职员工，并保证每周休2天
    const fullTimers = scheduleData.filter(
      (row) => row.employee.type === 'FullTime',
    );
    weekDays.forEach((day) => {
    ...
    const partTimers = scheduleData.filter(
      (row) => row.employee.type === 'PartTime',
    );
    ...
    weekDays.forEach((day) => {
      partTimers.forEach((row) => {
        ...
        if (Math.random() > 0.4) {
          // 60%的概率安排上班
          const shiftType =
            partTimeShifts[Math.floor(Math.random() * partTimeShifts.length)];
          row[day] = shiftType;
        }
      });
    });
...
<template>
  <div class="scheduling-container p-4">
      ...
      >
        <template #bodyCell="{ column, record }">
          ...
          <template v-if="column.key === 'employeeInfo'">
            ...
            <a-tag
              :color="record.employee.type === 'FullTime' ? 'blue' : 'green'"
            >
